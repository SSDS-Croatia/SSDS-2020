...
def get_features(graph, rep_method, verbose = True):
	...
	for n in range(num_nodes):
		for layer in list(graph.khop_neighbors[n].keys()): #construct feature matrix one layer at a time
...
def xnetmf(adj, dim = 128):
	graph = Graph(adj)
	rep_method = RepMethod(max_layer = 2, p = dim, normalize = True)
