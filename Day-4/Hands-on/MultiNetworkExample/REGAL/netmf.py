...
def netmf_mat_full(A, window = 10, b=1.0):
    ...
    Y = np.log(np.maximum(M.toarray(),1)) #just do dense elementwise log calculations because this isn't a scalable method anyway
    #m = T.matrix()
    #f = theano.function([m], T.log(T.maximum(m, 1)))
    #Y = f(M.todense().astype(theano.config.floatX))
    return sparse.csr_matrix(Y)

#Approximate NetMF (for large window size) TODO add
def netmf_mat_approx(A, window=10, b=1.0):
    pass
